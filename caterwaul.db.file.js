...
// filehandles and causing the HTTP server to bomb out.) Requests are serviced in the order made.
  ...
  let*[settings = this[caterwaul].util.merge({mode: 0755, filehandles: 128, memory_index: true}, options || {}),
       queue = this[caterwaul].clone('queue.blocking').queue.blocking, fh_queue = queue(settings.filehandles), handle_queues = {}, cached_filehandles = {},
       ...
       append(file, data, cc) = let*[mkdir_p(dir, cc)    = path.exists(dir, fn[b][b ? cc && cc() : mkdir_p(path.dirname(dir), fn_[fs.mkdir(dir, settings.mode, cc)])]),
                                     write_data(free_cc) = let[w = cached_filehandles[file]] in (w.write(data, 'utf8'), w.end(fn_[free_cc(), cc && cc()]))]
                                [mkdir_p(path.dirname(file), fn_[fh_queue(fn[free_cc]
                                  [handle_queues[file] || (handle_queues[file]      = queue(1, {empty: function () {free_cc(); delete handle_queues[file], cached_filehandles[file]}}),
                                                           cached_filehandles[file] = fs.createWriteStream(file, {flags: 'a+'})),
                                   handle_queues[file](write_data)])])],
  ...
  fn[id][id = id || this[caterwaul].gensym(), fn[x][x === undefined ? id : x.constructor === String   ? index_append(x, id) :
